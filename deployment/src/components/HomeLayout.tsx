...
export const HomeLayout: React.FC<HomeLayoutProps> = ({
  ...
  const currentPlatformData = data.length > 0 ? data[carouselIndex % data.length] : selected;
        ...
        <div className="overview-card trend-card">
          <div className="overview-header">
            <div>
              ...
              {showChartSkeleton ? (
                <Skeleton width={180} height={24} borderRadius={6} style={{ marginTop: 6 }} />
              ) : (
                <>
                  <h2>{currentPlatformData?.platform ?? "—"} trend</h2>
                  <p className="overview-subtitle">
                    Chart bars adjust to metric context for cleaner data representation.
                  </p>
                </>
              )}
            </div>
          ...
          {showChartSkeleton ? (
            <div className="chart-wrap">
              <Skeleton height={220} borderRadius={10} style={{ marginTop: 14 }} />
            </div>
          ) : (
            <>
              <div
              ...
              <div className="carousel-controls">
                <button className="carousel-btn" onClick={handleCarouselPrev}>←</button>
                <span className="carousel-counter">
                  {data.length > 0 ? `${carouselIndex + 1} / ${data.length}` : "— / —"}
                </span>
                <button className="carousel-btn" onClick={handleCarouselNext}>→</button>
              </div>
            </>
          )}
        </div>
          ...
          {showSummarySkeleton ? (
            <ul className="summary-list">
              {Array.from({ length: 5 }).map((_, i) => (
                <li key={i} className="summary-item">
                  ...
                  <Skeleton width="80%" height={13} borderRadius={6} style={{ flex: 1 }} />
                </li>
              ))}
            </ul>
          ) : (
            <ul className="summary-list">
              {summaryItems.map((item, i) => (
                <li key={i} className={`summary-item summary-priority-${item.priority}`}>
